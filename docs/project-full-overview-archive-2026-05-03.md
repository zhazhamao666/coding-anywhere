...
> 这份文件保留 `docs/project-full-overview.md` 深度精简前的完整长文档，主要用于历史溯源和细节查证。当前实现、当前边界和推荐验证路径以 [项目总览](./project-full-overview.md) 为准。
...
飞书“话题(topic)”、群 `thread_id` 主题相关旧实现和底层线程绑定记录不能被视为当前产品化 surface 或真实 UI 回归 surface；当前没有可用于自动真实回测的专用话题夹具，也没有把话题做成清晰的用户主入口。
...
- 长任务有实时状态，不是黑盒
- 同一个飞书会话窗口可以复用长期存在的 Codex 会话
- 多个 Codex 线程可以并行跑，但同一线程内不会并发污染上下文
...
4. DM 与项目群主时间线级会话绑定
5. 基于 native `thread_id` 的 DM / 群主时间线执行解析；飞书话题 / 群 `thread_id` 主题只保留历史底层兼容，不作为产品化入口
...
14. `/ca thread create*` 这类会创建飞书主题的历史命令现在会直接返回“不支持创建飞书主题”，不再作为可用入口
15. 在群主时间线通过 `/ca project bind-current` 直接绑定当前群
16. 在已绑定项目群主时间线中通过 `/ca thread list-current`、`/ca thread switch <threadId>`、`/ca new` 以及普通群消息直接操作当前项目会话
...
20. DM 中可以从 Codex 项目列表进入会话列表，也可以先把“当前这个飞书聊天窗口”切到某个 Codex 项目，再决定是否查看线程或创建新会话；手输切换命令时可使用真实 `projectKey` 或唯一项目显示名，bridge 会归一化保存真实 catalog key
...
30. 导航卡、项目列表卡、会话列表卡、当前项目卡等会使用各自的卡片标题，不再统一显示为 `CA Hub`
...
37. 长任务在飞书中的终态展示现在收敛为“终态卡 + 完整正文消息”：终态卡会直接内嵌 `Codex 最终返回了什么` 的收敛正文、本次/下次任务设置，以及 `新会话 | 切换会话 | 更多信息` 后续动作；完整 assistant 正文仍单独作为普通消息 / 线程回复发送，避免卡片和消息同时完整重复同一大段结果
38. `/ca new` 不再调用 Codex 模型，也不立即创建飞书主题；它只清理当前 DM / 已绑定项目群主时间线的会话绑定并进入“已选项目但未绑定会话”的 B 场景，下一条普通消息才会创建新的 native Codex thread
39. `/ca stop` 现在会按当前 DM / 已绑定项目群主时间线查找 live run：排队中的 run 直接取消并收口为 `canceled`，运行中的 run 会先进入 `canceling` 再终态收口
40. `thread list-current` 在已绑定项目群中会直接列出当前项目对应的 Codex native thread
41. `/ca thread switch <threadId>` 可用于 DM 和已绑定项目群主时间线：在群里会把“当前群对话”直接绑定到选中的 native thread；飞书主题 surface 上的切换请求会被拒绝
...
45. 飞书卡片回调现在显式分型：导航、状态、项目、会话列表/切换和设置类动作直接返回 `raw card` 即时替换；`/ca new` 返回 toast + raw card，提示下一条消息会开启新会话；计划表单提交与计划选项点击则先返回 toast，再在当前 surface 下新发一条进度卡消息续跑，不再 patch 被点击的卡
...
48. 飞书 DM 和已绑定项目群主时间线现在都可以先发送图片；bridge 会把图片下载为本地受管资产，并按当前飞书 surface 暂存，而不是立刻触发 Codex
49. 同一个 DM / 群主时间线 surface 上，下一条普通文本消息会自动消费这些待处理图片，并通过 `codex exec -i ...` 或 `codex exec resume -i ...` 一起送入 Codex
...
56. Codex 会话列表卡会把 subagent 来源解析为结构化的母 agent / 子 agent 展示，按父线程分组缩进显示 agent 名称、角色、父线程和层级，不再把 Codex raw `source` JSON 原样暴露到飞书卡片里
...
60. `/ca` 与 `/ca session` 现在会先按 surface 的就绪状态分流：未选项目时返回只含 `查看项目` 的起始卡；已选项目但未绑线程时返回项目作用域入口卡（`选择会话 | 新会话 | 查看项目`）；只有真正已绑定 native thread 的 surface 才会进入“当前会话已就绪”稳定态主卡
...
74. Codex 偏好现在按“当前线程优先、当前 surface 兜底、系统默认回退”的顺序生效：已绑定 native thread 的 DM / 项目群主时间线会把设置记到 `thread_id` 级别；尚未绑定 native thread 的 DM、项目群或待创建会话入口则先记到当前 surface，并在后续创建新线程时继承
...
79. Playwright 版真实飞书 live smoke 现在默认锁死到 `coding-anywhere-autotest` 夹具：`test:feishu:live` / `test:feishu:live:dm` 会先把测试 DM 切到该项目，`test:feishu:live:group` 只允许命中已绑定好的测试群主时间线 `coding-anywhere-autotest`；`test:feishu:live:dm:ui` 和 `test:feishu:live:group:ui` 会运行完整 UI journey 矩阵；当前真实 UI 回归不支持 `topic`、飞书话题、群 `thread_id` 主题或 handoff 场景，如确实需要覆盖到别的项目或群夹具，必须显式设置危险开关 `FEISHU_LIVE_ALLOW_NON_AUTOTEST=1`
80. 桌面 completion 通知卡的主按钮文案现在统一为“在飞书继续”，当前产品化接管路径以 DM 和项目群主时间线为准：目标 native Codex thread 会接到对应飞书 surface，并统一落到“当前会话已就绪”稳定态主卡；项目群主时间线不会自动创建飞书话题，而是直接把当前群对话绑定到该 native thread
81. 飞书已绑定项目群主时间线现在和 DM 保持同一套心智：`切换到此会话` / `在飞书继续` 都会把当前对话窗口直接绑定到一个 native Codex thread，后续普通群消息会继续进入这个线程
...
87. 会话切换卡、会话绑定卡和“当前会话”卡里的最近对话预览，现在也会对 assistant 文本复用同一套 Codex app directive 可见性规则：`::git-stage` / `::git-commit` 这类 app 指令不会再原样显示到飞书卡片里，但若对应仓库可读，仍会保留 `N 个文件已更改` 这种紧凑摘要
88. “当前会话已就绪”稳定态主卡现在只在真正已绑定 native thread 的 surface 上出现：首屏只保留项目 / 线程 / 状态 / 作用范围四条上下文、常驻的下次任务设置下拉、计划模式状态项，以及 `切换会话 | 更多信息` 两个后续动作，不再在主卡首屏平铺线程 ID、群聊 ID、诊断字段或旧版大导航按钮组
89. 新群主时间线或未选项目的 DM 在 `/ca` 首屏不再错误展示计划模式、下次任务设置、`切换会话` 或 `更多信息`；这些控件只有在用户已经进入具体线程后才会出现，避免出现“看得见但根本不能完成下一步”的假入口
90. 群主时间线里的项目列表底部动作现在也会按当前群是否已绑定项目来收紧：未绑定项目时不再展示会失败的 `新会话`，只保留返回入口；绑定项目后才恢复 `新会话`
91. DM / 已绑定项目群里的 Codex 项目列表和会话列表现在开始改用“选择卡”版式：项目行统一只有 `进入项目` 一个主动作，线程行统一只有 `切换到此会话` 一个主动作；路径、raw source、完整 `thread_id` 和分支等技术字段会从首屏列表中移除，底部动作也统一收敛为“返回当前会话/返回导航 + 新会话”；会话列表会限制为最近 12 条并截断长标题/长身份行，确保延时更新卡片保持在飞书卡片大小限制内
...
101. 新生成的飞书卡片 action value 会携带 `actionKind`，把命令、会话 UI、计划选择、桌面接管和偏好设置动作显式分型；回调归一化也会保留新版 `card.action.trigger` 中的 `open_chat_id`、`options`、`checked`、`input_value`、`host` 和 `timezone` 等字段，避免协议边界继续被按钮场景锁死
102. 桌面 completion 的 DM fallback 现在不再强制要求配置 `feishu.desktopOwnerOpenId`：bridge 会优先使用目标 native thread 已绑定的 DM 用户，其次使用单人 `allowlist`，最后使用本地唯一已见 DM 用户；只有出现多个 DM 候选且无法从线程绑定判断时才继续要求显式配置
...
也就是说，当前产品化入口只按 DM 与已绑定项目群主时间线验收；“群聊话题 / topic / 群 `thread_id` 主题承载会话”的完整产品方案留待后续单独设计。
...
- `Codex 会话`
  - 长期存在
- DM 与已绑定项目群主时间线最终都可以绑定到 native `thread_id`
...
- 按目标类型分别执行 DM 和项目群主时间线投递；正常路由不会选择飞书主题作为投递目标
- 在真正发飞书消息之前先校验 `codex_thread_watch_state` 是否已存在，避免“消息已经发出但 watch-state 持久化才报错”的半成功状态
- 如果数据库里残留历史飞书主题投递状态，这类记录只按兼容数据处理，不进入产品化路由或 live journey
...
- 文本消息过滤
- 图片消息下载与 surface 级暂存
- DM 与已绑定项目群主时间线 surface 识别；群 `thread_id` 主题消息会在入口处丢弃
...
- 为计划模式开关、诊断卡切换和计划选择按钮编码 bridge 动作上下文
- 为未来的桌面 completion 通知提供纯本地路由解析：优先当前产品化的项目群主时间线或 DM 绑定；历史话题绑定只作为 stale 数据负向守护，不进入路由或 live journey；cwd 命中多个项目时不会猜测路由目标
- 处理桌面 completion 的 continue 接管：当前产品化路径以 DM 和项目群主时间线为准，都会把目标 native thread 接到对应飞书 surface，并统一复用“当前会话已就绪”稳定态主卡；项目群主时间线会直接把当前群对话绑定到目标 thread，而不是创建新话题
- root 上下文封装
...
- 统一编码 `/ca` 命令按钮、计划模式开关、诊断卡切换、计划选择按钮、Codex 设置回调和桌面线程接管按钮的 `value`
...
- 历史飞书主题相关 helper 只用于遗留数据和测试保护；当前产品入口不会创建或绑定飞书主题
- 创建 `codex_threads` surface 绑定记录

注意：

- 这个服务已经存在
- 当前 `/ca thread create*` 会返回“不支持创建飞书主题”，不作为产品化 UI 或 live 回归入口
- 但还没有自动建群或图形化创建流
...
- 群 `thread_id` 主题消息当前不进入 bridge 任务链路；产品化群聊入口使用群主时间线
...
- 统一桌面 lifecycle 通知卡的主按钮文案为“在飞书继续”，并把 DM、项目群主时间线接管场景编码进按钮 payload；历史话题只作为不支持入口的负向守护
...
- 统一产出后续接管会复用的稳定动作名，如 `continue_desktop_thread`、`view_desktop_thread_history`、`mute_desktop_thread`
...
- 如果数据库里的 `codex_threads` 仍以 `thread_id` 作为主键，启动迁移会自动重建为“按飞书 surface 建模”的新结构；其中多个话题绑定到同一个 native `thread_id` 只属于历史兼容数据模型
...
本小节只记录历史底层模型，避免误把旧实现当作当前能力。当前产品化和真实 live 回归只覆盖 DM 与已绑定项目群主时间线，不覆盖飞书 topic / 话题 / 群 `thread_id` 主题。
...
来解析上下文；它不属于当前推荐真实联调 surface。

如果同一个 `(chat_id, thread_id)` 上已经暂存过图片，则下一条历史话题文本消息会先消费这些图片，再续跑当前 native `thread_id`。

## 6.2.1 图片暂存链路

```text
Feishu DM / Bound Group Main Timeline image
  -> FeishuAdapter
...
本地收到群消息后，当前产品化路径只有满足下面任一条件才会进入 Codex：
...
若开启 `feishu.requireGroupMention`，则群主时间线消息必须带 mention 才会进入 Codex；该开关不会让飞书主动推送普通群消息，推送范围仍由飞书后台权限决定。
...
- DM 中 `/ca project list`
  ...
  - 展示 Codex 派生项目列表
  - 每个项目行只展示一个主动作按钮，当前默认展示“进入项目”；如需查看线程，可先进入项目后再进入会话列表
...
- 群主时间线中的 `/ca project list`
  - 如果已配置 Codex catalog，也展示 Codex 派生项目列表
  - 群主时间线中会标出每个项目的群绑定状态，并允许点击未绑定项目直接绑定当前群
  - 如果没有 Codex catalog，则回退展示 CA 本地注册的项目列表
- 已绑定项目群主时间线中的 `/ca thread list-current`
  - 通过当前项目的 `cwd` 对齐到 Codex catalog project
  - 直接列出该项目下的 native Codex thread

这些命令现在只按 DM 与已绑定项目群主时间线两类产品化 surface 验收。

其中：

- DM 中 `/ca new` 会清理当前窗口的旧 thread 绑定，并停在“已选项目但未绑定会话”的 B 场景；下一条普通 prompt 才会创建新的 native thread
- 如果 DM 当前没有 native thread 绑定但已经选中了项目，`/ca new` 与下一条普通 prompt 都会优先使用该项目的 `cwd`
- 已绑定项目群主时间线中的 `/ca new` 会清理当前群对话的旧 thread 绑定，并停在 B 场景；下一条普通群消息才会创建新的 native thread
...
- `/ca` 会按上下文返回不同内容，`/ca hub` 复用同一条路径：
  ...
  - 已绑定项目群主时间线：未绑定会话时显示当前项目与会话选择入口，已绑定会话后显示“当前会话已就绪”
- 如果当前 surface 有 live run，`/ca` 会直接把当前运行摘要嵌进卡片里；没有 live run 时不会再显示“停止任务”
- `/ca` 的按钮会按上下文变化：
  - DM：`导航`、`运行状态`、`当前会话`、`新会话`、`计划模式`、`项目列表`，有 live run 时额外出现 `停止任务`
  - 已切到 Codex 原生线程的 DM：`导航`、`项目列表`、`当前项目`、`会话列表`、`计划模式`、`当前会话`、`运行状态`、`新会话`，有 live run 时额外出现 `停止任务`
  - 已绑定项目群主时间线：未绑定会话时保留 `选择会话`、`新会话`、`查看项目`；已绑定会话后复用 DM 稳定态的 `计划模式`、`切换会话`、`更多信息`
...
- `/ca thread list <projectId>` 与 `/ca thread list-current` 会返回会话列表卡片；在 DM 中若当前没有 native thread 绑定，则 `thread list-current` 会回退到当前所选项目
- 会话列表卡现在改用“选择卡”版式：首屏只展示线程标题、主线程 / 子 agent 标记、最近更新时间，以及必要时的父线程引用与层级，不再在列表首屏直接暴露 raw `thread_id`、来源 JSON 或分支字段
...
- 已选项目但还没绑定线程时，`/ca` / `/ca session` 会返回项目作用域入口卡：只保留 `选择会话 | 新会话 | 查看项目`
...
- `/ca thread create*` 当前会直接返回“不支持创建飞书主题”
- DM 中的项目列表卡和会话列表卡现在带行级按钮：项目列表每行只保留一个主动作“进入项目”，会话列表每行只保留一个主动作“切换到此会话”；列表底部动作也统一收敛为“返回当前会话/返回导航 + 新会话”
...
- 一个已绑定 DM 或项目群主时间线，对应一个 native Codex thread 绑定
- 同一个飞书会话后续消息继续复用该 native `thread_id`
- 历史飞书主题绑定记录可能仍存在于 SQLite 中，但这不是当前产品化入口或 live surface；新交互不会把飞书主题作为可用会话入口
...
- `codex_threads` 在历史话题兼容路径中以 `(chat_id, feishu_thread_id)` 唯一标识一个飞书话题 surface，而不是再把 `thread_id` 当作唯一主键
- 因此同一个 native `thread_id` 可以被多个历史飞书话题引用；项目摘要中的线程数按去重后的 native `thread_id` 统计
...
- `feishu.requireGroupMention`
  - 群主时间线兜底模式
  - 为 `true` 时，只有带 mention 的群主时间线消息才会进入 Codex；mention 依据飞书官方 `message.mentions` 字段识别
  - 该配置只控制本地过滤，不替代飞书后台权限；若应用只有群 @ 机器人消息权限，未 @ 的普通群消息不会被飞书推送到长连接
- `feishu.desktopOwnerOpenId`
  - 桌面 completion 通知在无法路由到项目群主时间线时，用于显式指定 DM fallback 的目标用户 `open_id`
...
- `npm run test:feishu:live` 与 `npm run test:feishu:live:dm` 会复用该 profile 打开真实飞书测试 DM；`npm run test:feishu:live:group` 会复用同一 profile 打开真实飞书测试群主时间线；`npm run test:feishu:live:dm:ui` 与 `npm run test:feishu:live:group:ui` 会执行完整 UI journey 矩阵，不再重复自动登录；当前没有 `topic` live 脚本
...
- 常规 DM / group 主时间线旅程应从用户自然入口开始：通常先发送 `/ca`，再根据返回卡片点击 `查看项目`、`当前项目`、`选择会话`、`会话列表`、`返回当前会话` 等按钮继续；如果某一步要验证卡片交互，就优先点击卡片按钮，而不是直接发送等价命令
- 专项 live 测试可以先用 `/ca project switch`、预置会话等方式构造上下文，但用例名称和步骤应说明它是在验证特定功能点，而不是普通用户导航路径；桌面 handoff、topic / 话题和群 `thread_id` 主题不属于当前 live 支持场景
- 每一步断言必须等待当前动作产生的新可见结果，不能只因为历史消息里已有同名文案就提前通过；否则无法发现卡片未刷新、点击旧卡、延时更新失败这类真实交互问题
- live smoke 不再只发一条可配置命令，而是按 surface 执行主要用户旅程；其中夹具准备步骤会和用户主旅程分开记录。常规 DM 会先用 `/ca project switch coding-anywhere-autotest` 做夹具准备，然后主旅程从 `/ca` 开始，点击 `查看项目`、从项目列表返回当前会话、点击 `选择会话` 进入会话选择，再查看 `/ca status` 与 `/ca session`；常规群主时间线会先用 `/ca project current` 做夹具自检，然后主旅程从 `/ca` 开始，查看项目列表、点击 `当前项目`、点击 `会话列表` 进入会话选择与查看 `/ca status`
- 完整 UI journey 可通过 `FEISHU_LIVE_SCENARIOS=all` 或 `*:ui` 脚本触发，会覆盖标准会话卡、`更多信息` 诊断卡、计划模式开关、新会话、会话切换、短任务终态和 `/ops/ui`；这些场景会在 autotest DM / 测试群主时间线内创建或复用测试会话，仍禁止自动改绑非测试群
...
- `FEISHU_LIVE_SURFACE` 用于显式指定当前 smoke 场景：`dm` 或 `group`；这里的 `group` 固定表示已绑定项目群主时间线，不表示飞书 topic / 话题 / 群 `thread_id` 主题。显式传 `topic` 会直接报错，因为当前没有可用于真实 UI 回归的话题夹具
- `FEISHU_LIVE_SCENARIOS` 可用逗号指定要执行的 UI 场景；常用值为 `main` 或 `all`。`topic`、`handoff` 和飞书话题链路都不是支持场景
- `FEISHU_LIVE_DM_CONVERSATION_NAME` 可在 DM 只拿到 messenger 根页 URL 时指定左侧机器人 DM 会话名；本机默认测试 DM 是 `渣渣Co`
- `FEISHU_LIVE_CONVERSATION_NAME` 是通用覆盖项，可在 `FEISHU_LIVE_TARGET_URL` 只能打开 messenger 根页时指定左侧会话名；若 `FEISHU_LIVE_SURFACE=group` 且未显式提供，则默认固定为测试群 `coding-anywhere-autotest`；未开启危险开关时，group smoke 也会拒绝任何其他群名
...
- 已绑定项目群主时间线中的消息可以直接进入 Codex
- 群主时间线结果不再回 DM，而是在当前群会话里回复
- 可以在 DM 或群主时间线使用 `/ca new` 退出当前会话，下一条普通消息会在当前项目下创建新的 native Codex thread
- 可以直接从飞书命令注册现有项目群
- 可以在已绑定项目群里查看当前项目会话列表并切换当前会话
- 可以在群主时间线快速确认当前项目绑定
...
- 在项目群主时间线把现有 Codex 原生线程绑定到当前群会话时，回执卡会明确进入“当前会话已就绪”，并显示最近对话预览
- 在 DM 中切换项目后，旧线程绑定会被立即解除；这时下一条普通消息会在新项目下创建 fresh thread，而不是继续跑旧项目的上下文
- 可以在 DM 或已绑定项目群主时间线里直接点击“运行状态”，看到当前 surface 的 live run、耗时、最近工具和摘要；没有 live run 时也能看到空闲态上下文摘要
- 可以在 DM 或已绑定项目群主时间线里直接点击“停止任务”或发送 `/ca stop`，请求停止当前 surface 上正在执行或排队的任务
...
- 可以直接点击卡片按钮回到导航、当前项目和会话列表，而不用重新手输命令
- 当卡片同时展示“当前线程”和 `Session` 时，当前线程现在只显示线程名称，不再重复展示同一个 native `thread_id`
- 可以在 DM 和已绑定项目群主时间线的“当前会话已就绪”卡里直接切换 `计划模式 [开/关]`，让下一条普通消息按 `/plan ...` 方式发起
...
- 可以先在 DM 或已绑定项目群主时间线里发送图片，再补一条文字说明；bridge 会自动把这些图片带进下一次 Codex run
...
- 桌面 completion 通知虽然已经有本地路由决策、DM owner 配置解析、实际消息发送器、runtime 轮询，以及 DM / group continue 接管，但 history/mute 回调以及“同一 completion 失败后自动修复”还没有接通
...
10. 再发一个普通任务，确认 DM 中先出现流式状态卡；run 完成后，卡片收口为终态卡，并直接显示 `Codex 最终返回了什么` 的收敛正文、`新会话 | 切换会话 | 更多信息` 动作，以及下次任务设置；完整 assistant 正文以下方单独消息展示
...
14. 飞书 DM 或已绑定项目群主时间线里点击 `/ca session`、`/ca status`，再启动一个普通任务，确认三种卡片都会显示当前 `model` / `reasoning effort` / `speed`，并能通过下拉框切换
...
飞书 topic / 话题 / 群 `thread_id` 主题不属于当前推荐真实回归路径，也没有专用 live 夹具；除非先单独设计并确认新夹具，否则不要把它加入 `npm run test:feishu:live*` 或手工真实 UI 回测。
...
4. 在已绑定项目群主时间线里重复上述流程，确认群主时间线 surface 也能复用同样的暂存与消费逻辑
...
7. 在已绑定项目群主时间线里重复以上流程，确认群主时间线 surface 也能复用相同链路
...
3. DM 场景执行 `npm run test:feishu:live` 或 `npm run test:feishu:live:dm`；已绑定项目群主时间线场景执行 `npm run test:feishu:live:group`；当前不提供真实话题 smoke
4. DM smoke 会复用 `.auth/feishu-profile` 打开真实测试 DM，先执行 `/ca project switch coding-anywhere-autotest` 做夹具准备；随后用户主旅程从 `/ca` 开始，依次点击 `查看项目`、`返回当前会话`、`选择会话`，再检查 `/ca status` 与 `/ca session`
5. group smoke 会复用同一 profile 打开真实测试群，默认群名固定为 `coding-anywhere-autotest`；脚本会先执行 `/ca project current` 校验当前群已绑定到 `coding-anywhere-autotest`，再从 `/ca` 开始查看项目列表、点击 `当前项目`、点击 `会话列表`、检查 `/ca status`，不会自动改绑
6. 完整 UI 回归执行 `npm run test:feishu:live:dm:ui`、`npm run test:feishu:live:group:ui` 或设置 `FEISHU_LIVE_SCENARIOS=all`，会额外覆盖稳定态会话卡、诊断卡、计划模式开关、新会话、会话切换、短任务终态和 `/ops/ui`
7. 各类 smoke 都会默认拒绝非 `coding-anywhere-autotest` 项目；group smoke 还会额外拒绝非默认测试群 `coding-anywhere-autotest`。如确实需要覆盖到别的项目或群夹具，必须显式设置 `FEISHU_LIVE_ALLOW_NON_AUTOTEST=1`
8. smoke 还会请求 `/ops/overview` 确认本地 bridge 控制面可达；如需覆盖地址，设置 `FEISHU_LIVE_OPS_BASE_URL`
9. 如需追加一条额外 smoke 指令，可设置 `FEISHU_LIVE_SMOKE_TEXT` 与 `FEISHU_LIVE_EXPECT_TEXT`；如需调整会话选择或输入框定位，可设置 `FEISHU_LIVE_DM_CONVERSATION_NAME`、`FEISHU_LIVE_CONVERSATION_NAME`、`FEISHU_LIVE_COMPOSER_SELECTOR`
10. 登录态失效时，重新运行 `npm run test:feishu:auth` 刷新 profile
11. 如果某个问题暂时没有被现有 smoke 覆盖，需要在任务执行过程中临时追加真实联调，也必须沿用同一套规则：DM 先确认或切换到 `coding-anywhere-autotest`，群聊先确认群名和 `/ca project current` 都指向 `coding-anywhere-autotest`；不满足条件时应先停下补夹具，而不是直接在业务会话里验证
...
**一个支持 DM / 已绑定项目群主时间线、文本与图片桥接、Codex 线程级会话、run 级 worker、会话回推和项目/线程/run 三层观测的单实例桥接服务。**
