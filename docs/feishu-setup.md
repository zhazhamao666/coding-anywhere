...
- inbound 图片/文件不会走单独事件类型，仍然通过同一条“接收消息”链路进入服务
- bridge 会从消息内容里解析 `image_key` 或 `file_key`，再调用“获取消息中的资源文件”接口下载资源
...
如果你要启用“飞书图片/文件 <-> Codex”链路，还要额外确认：

1. 应用已经具备读取消息资源的权限，否则 bridge 拿不到 inbound 图片/文件二进制。
2. 应用已经具备上传图片、发送图片消息的权限，否则 outbound 图片会退回文件消息或可见失败说明。
3. 应用已经具备上传文件、发送文件消息的权限，否则普通文件、超限图片、`.md` 和 `.drawio` 都无法回传为原生文件消息。
4. 发送给飞书的单张图片需要满足飞书原生图片接口限制；当前实现按官方限制以 `10MB` 作为原生图片可发送上限，超限但不超过 `30MB` 的图片会降级为文件消息。
5. 发送给飞书的单个文件需要满足飞书文件接口限制；当前实现以 `30MB` 作为文件可发送上限，超过后只回可见错误说明，不会上传。

Markdown(`.md` / `.markdown`) 和 draw.io(`.drawio` / `.drawio.xml`) 不需要飞书专用消息类型，走普通 `file` 消息。bridge 会把它们标记为语义文件交给 Codex，也支持按源文件回传；自动 Markdown 预览卡和 draw.io 渲染预览仍是后续增强。
...
1. 飞书 DM 先发送一张图片
2. 确认只收到“已收到图片，请继续发送文字说明”
3. 再发送 `test`
4. 确认这次 run 会带上刚才暂存的图片
5. 再发送一个 `.md` 文件
6. 确认只收到“已收到文件：<文件名>，请继续发送文字说明”
7. 再发送 `test`
8. 确认这次 run 的附件提示里包含该文件的 `local_path` 和 `semantic_type=markdown`

资源双向链路可用下面的专项 smoke 覆盖：
...
这两个 smoke 会使用 `coding-anywhere-autotest` 夹具，覆盖 inbound Markdown 附件、Codex 生成 PNG / `.md` / `.drawio`，以及通过 `[bridge-assets]` 回传到飞书。

如果你还需要群主时间线能力，再继续按 [项目总览](./project-full-overview.md) 里的“验证路径”做群主时间线回归。当前真实联调不覆盖飞书 topic / 话题 / 群 `thread_id` 主题。

启动方式和长期运维见 [管理员部署手册](./admin-deployment.md)；如果最小联调失败，先查 [故障排查手册](./troubleshooting.md)。
...
- 图片或文件消息能收到，但应用没有“读取消息资源”能力，导致服务拿不到资源文件
- Codex 已经产出本地图片，但应用没有“上传图片 / 发送图片消息”能力；若文件能力可用，bridge 会尝试按文件回传，否则只能给出失败说明
- Codex 已经产出本地文件，但应用没有“上传文件 / 发送文件消息”能力，导致普通文件、`.md`、`.drawio` 或超限图片无法回传
- 生成图片超过飞书原生图片消息可接受的大小；不超过 `30MB` 时会作为文件发送，超过 `30MB` 时不会上传
- 期望 `.md` 或 `.drawio` 自动渲染成预览卡/图片；当前只保证源文件传输和语义标记，预览渲染仍是后续增强
