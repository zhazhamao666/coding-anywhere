...
本设计初稿只基于源码、测试和官方飞书文档分析现状；初稿阶段没有读取或修改 `docs/project-full-overview.md`。功能落地和验证完成后，已按项目规则同步修订总览及相关项目文档。

## 1.1 落地结果

本轮已落地：

- 飞书 inbound 图片/文件统一暂存，下一条文本消息自动带入 Codex。
- 图片继续通过 Codex CLI 图片参数输入；普通文件通过 `[bridge-attachments]` 元数据把 `local_path`、文件名、MIME、大小、来源消息和语义类型交给 Codex。
- `.md` / `.markdown` 标记为 `semantic_type=markdown`，`.drawio` / `.drawio.xml` 标记为 `semantic_type=drawio`。
- assistant 可用统一 `[bridge-assets]` 指令回传图片和文件；旧 `[bridge-image]` 继续兼容。
- 出站资源路径收紧到当前 run 专属输出目录、desktop completion 专属输出目录，或本轮已消费入站附件的 exact `local_path`；不再允许任意项目 `cwd` 文件直接外发。
- 图片优先按飞书原生图片发送；超过原生图片限制或图片接口不可用时，在文件能力可用且不超过文件上限的情况下退回文件消息。
- 资源本地路径在飞书可见文本、进度、错误、caption 和文件名链路中做脱敏，避免把临时路径暴露给飞书用户。
- runtime maintenance 会清理过期、失败或已消费的 bridge 资源文件。

本轮未落地、保留为后续增强：

- 自动 Markdown 预览卡。
- 自动 draw.io PNG/SVG/PDF 渲染预览和 `DrawioPreviewExporter`。
- 独立出站资源审计表。
...
- 文件安全边界不再沿用早期“当前项目工作目录或桥接托管资源目录”宽边界；出站只允许当前 run/desktop completion 专属输出目录内的文件，或本轮已消费入站附件的 exact `local_path`。
- Markdown 文件指 `.md` / `.markdown`，作为普通文件传输，但在提示、预览和验收中拥有明确语义；默认按 UTF-8 文本处理，MIME 缺失时回退为 `text/markdown`。
- draw.io 文件指 `.drawio` / `.drawio.xml`，作为可编辑图源文件传输；本轮先保证源文件传输和语义标记。飞书侧 PNG/SVG/PDF 自动预览属于后续增强，源文件仍必须一并返回，避免只给不可编辑快照。
...
- Markdown/draw.io 特化：不改变飞书底层传输形态，仍然走文件；本轮已新增轻量分类器，自动预览策略保留为后续增强，避免为少数格式改动数据库主模型。
...
- `kind` 允许 `image`、`file`。
- `path` 必填。回传新生成文件时应使用 `[bridge-output-assets]` 提供的 `root_path` 下的路径；回传本轮入站附件原文件时必须使用该附件的 exact `local_path`。
- `file_name` 可选；未提供时使用 `path.basename(path)`。
- `caption` 可选；用于发送前后的文本说明，不写入文件内容。
- `presentation` 可选，允许 `attachment`、`markdown_preview`、`drawio_with_preview`；本轮先作为语义元数据保留并随文件 reply 传递，未提供时默认为 `attachment`。
- `preview` 可选，仅在 `presentation=drawio_with_preview` 时生效；自动预览导出属于后续增强，本轮不依赖该字段生成预览。
- 旧 `[bridge-image]` 继续兼容，内部可转换成 `kind=image` 的统一 asset。
- 若同一回复同时包含正文和资源，先发送正文/卡片，再发送资源。
- 若资源路径无效或越界，发送系统提示而不是静默失败。
- Markdown 预览不替代 `.md` 文件上传；draw.io 预览不替代 `.drawio` 源文件上传。本轮验收以源文件传输和语义标记为准。

### 8.4 出站路径校验

允许路径：

- 当前 Codex run 的专属输出目录，即 prompt 中 `[bridge-output-assets] root_path` 指向的位置。
- desktop completion 的专属输出目录。
- 本轮已消费入站附件的 exact `local_path`。

拒绝路径：

- 任意项目 `cwd` 中未位于专属输出目录的文件。
...
- 本轮 `presentation=markdown_preview` 仅作为语义元数据保留，不自动生成预览卡。
- 后续若实现自动预览，预览内容必须来自文件正文或 Codex 已给出的 caption/summary，不包含 `[bridge-assets]` 等桥接指令。
- 文件较大、解析失败或内容包含过多代码块时，应降级为仅发送 `.md` 文件，并用 caption 说明。
...
- 本轮 `presentation=drawio_with_preview` 仅作为语义元数据保留，不自动调用 draw.io CLI 导出预览。
- 后续实现自动预览时，桥接层可尝试把 `.drawio` 导出为 `preview.format` 指定的 PNG/SVG/PDF。
...
- 不自动上传未显式声明的文件。
- 不允许上传当前 run/desktop completion 专属输出目录和本轮已消费入站附件 exact `local_path` 之外的路径。
...
本节同时保留初稿测试目标和本轮实际验收结果。自动 Markdown 预览卡、draw.io 预览导出和 `DrawioPreviewExporter` 属于后续增强，不计入本轮完成标准。
...
- `BridgeService`：
  ...
  - `[bridge-assets]` 支持 `presentation=markdown_preview` / `drawio_with_preview` 作为语义元数据，并稳定回传源文件。
  - 自动 Markdown 预览和 draw.io 预览导出留作后续增强。
  - 旧 `[bridge-image]` 兼容。
  - 越界路径、目录、0 字节、大文件都有可读降级提示。
- `DrawioPreviewExporter`（后续增强）：
...
- 增补 live smoke：
  - `node scripts/feishu-live.mjs dm bridge-assets` 覆盖 DM 图片/文件资源双向链路。
  - `node scripts/feishu-live.mjs group bridge-assets` 覆盖 `coding-anywhere-autotest` 测试群图片/文件资源双向链路。
...
- Codex 回复 `[bridge-assets]` 声明 `.md` 且 `presentation=markdown_preview` 时，飞书收到 `.md` 源文件；自动可读预览留作后续增强。
- Codex 回复 `[bridge-assets]` 声明 `.drawio` 且 `presentation=drawio_with_preview` 时，飞书收到 `.drawio` 源文件；自动 PNG/SVG/PDF 预览留作后续增强。
...
- Markdown：本轮已实现语义识别和源文件回传；`markdown_preview` 自动预览留作后续增强。
- draw.io：本轮已实现语义识别和源文件回传；`DrawioPreviewExporter`、PNG/SVG/PDF 文件预览和 CLI 缺失降级留作后续增强。
- 补齐 md/drawio 的单元测试与 live smoke fixture，不依赖真实飞书环境验证自动转换逻辑。
...
- 增补 DM 与测试群 `bridge-assets` live smoke。
- 记录出站资源上传日志。
...
- Markdown 预览默认是否开启？本轮不开启自动预览；后续若实现，建议由 `presentation=markdown_preview` 显式开启，避免把长文档刷屏到飞书。
- draw.io 默认预览格式是否固定为 PNG？本轮不开启自动预览；后续若实现，建议默认 PNG，因为飞书可原生查看，SVG/PDF 仅在 Codex 指令或配置中显式指定。
