...
`切换线程` 与 `新会话` 都不再使用单独的“成功卡”。统一规则是：先给轻量 toast，再直接把当前卡替换成目标会话卡或下一张真正需要决策的业务卡。

#### 切换线程

交互流程：

1. 用户在 `[选择线程]` 中点击 `切换到此线程`
2. 立即展示 toast：`正在切换线程…`
...
1. 用户点击 `新会话`
2. 立即展示 toast：`正在创建新会话…`
3. 原卡直接替换成新线程的 `[当前会话已就绪]`
