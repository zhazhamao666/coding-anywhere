...
`modelOptions` 中的 GPT 家族模型会先归一化为小写 ID 供 CLI 使用，再在飞书里统一显示成 `GPT-*` 文案；下拉框会按 GPT 数值版本倒序排列，同版本再按 Codex / Base / Spark / Mini 等变体排序，非 GPT 自定义模型会保留原始 ID 并排在 GPT 家族之后。这样新增 `gpt-5.x` 模型时通常不需要再手工调整排序或大小写。
